...
##            FIGURE 8: SNOW CRAB ZYGOCARDIAC READABILITY
## PART II:  BAND COUNT
##      IIA: PRECISION
##            FIGURE 9: INDEPENDENT RED KING AND SNOW CRAB BAND COUNT ESTIMATES
...
# AMONG STRUCTURES
chisq.test(Red.Crab$Readable, Red.Crab$Structure)
    
# AMONG PAIRED (LEFT & RIGHT) 
  # EYESTALKS
  Red.Crab.Eye <- subset(Red.Crab, Structure == "eyestalk")
  chisq.test(Red.Crab.Eye$Readable, Red.Crab.Eye$Paired.Structure)  
  
  # UROMESOCARDIACS ARE UNPAIRED
 
  
  # PTEROCARDIACS
  Red.Crab.Ptero <- subset(Red.Crab, Structure == "pterocardiac")
  chisq.test(Red.Crab.Ptero$Readable, Red.Crab.Ptero$Paired.Structure)
  
  # ZYGOCARDIACS
  Red.Crab.Zygo <- subset(Red.Crab, Structure == "zygocardiac")
  chisq.test(Red.Crab.Zygo$Readable, Red.Crab.Zygo$Paired.Structure)


    # PROPORTIONS OF READABLE SECTIONS DO NOT SIGNIFICANTLY DIFFER AMONG STRUCTURES... 
    # USE ZYGOCARDIAC MOVING FORWARD DUE TO ITS PREVALENCE IN THE LITERATURE...
    # LEFT AND RIGHT PAIRED STRUCTURES DO NOT SIGNIFICANTLY DIFFER...
    # COMBINE LEFT AND RIGHT PAIRED STRUCTURES IN FURTHER ANALYSES.  
...
# AMONG STRUCTURES
chisq.test(Snow.Crab$Readable, Snow.Crab$Structure)
    
# AMONG PAIRED (LEFT & RIGHT) 

    # EYESTALKS 
      Snow.Crab.Eye <- subset(Snow.Crab, Structure == "eyestalk")
      chisq.test(Snow.Crab.Eye$Readable, Snow.Crab.Eye$Paired.Structure)  

    # UROMESOCARDIACS ARE UNPAIRED


    # PTEROCARDIACS
      Snow.Crab.Ptero <- subset(Snow.Crab, Structure == "pterocardiac")
      chisq.test(Snow.Crab.Ptero$Readable, Snow.Crab.Ptero$Paired.Structure)

    # ZYGOCARDIACS
      Snow.Crab.Zygo <- subset(Snow.Crab, Structure == "zygocardiac")
      chisq.test(Snow.Crab.Zygo$Readable, Snow.Crab.Zygo$Paired.Structure)
...
# AMONG PAIRED (LEFT & RIGHT) STRUCTURES
    chisq.test(Spot.Shrimp$Readable, Spot.Shrimp$Paired.Structure)
...
AIC(mod1$lme, mod3$lme, mod4$lme, mod5$lme, mod7$lme)
    ...
    ggplot(Crab2, aes(x=Thin.Section.Location, y=PredictedProb))+ 
      ...
      theme(axis.line = element_line(colour = "black"),
            ...
            text = element_text(size=20),
            panel.background = element_blank()) +
      labs(title="Snow Crab Zygocardiac", x="Location (%)", y="Readable (%)")
...
legend(x=-2, y= 23, bty="n", paste("APE", round(Reader4.stats[1,8], 1), "\n", "CV", round(Reader4.stats[1,6], 1))) 

par(mai=c(0.4,0.3,0.1,0.1))
plotAB(ab.tA8,what="bias",col.agree="gray50", xlim=c(0,19), ylim=c(0,19), xlab= "Average" , ylab="")
legend(x=-2, y= 23, bty="n", paste("APE",round(Reader8.stats[1,8], 1), "\n", "CV", round(Reader8.stats[1,6], 1))) 

mtext("Band Count Average Excluding Reader", side=1, cex= 1.5, outer=T, at=0.5)

mtext("Red King Crab", side = 3, cex= 1.2, line = -0.5, adj=0.25, outer = TRUE)
mtext("Snow Crab", side = 3, cex =1.2, line = -0.5, adj=0.80, outer = TRUE)
...
title(ylab="Reader 1", line=2, cex.lab=1.4, family="Times")
legend(x=0.2, y=12, bty="n", border=NULL, paste("APE",round(Reader9.stats[1,8],1), "\n", "CV", round(Reader9.stats[1,6], 1))) 

par(mai=c(0.35,0.55,0.05,0))
plotAB(ab.tA10,what="bias",col.agree="gray50", xlim=c(1,9), ylim=c(1,11), xlab= "", ylab=" ")
title(ylab="Reader 2", line=2, cex.lab=1.4, family="Times")
legend(x=0.2, y=12,bty="n",paste("APE", round(Reader10.stats[1,8], 1), "\n", "CV", round(Reader10.stats[1,6], 1)))  

par(mai=c(0.35,0.5,0.05,0))
plotAB(ab.tA11,what="bias",col.agree="gray50", xlim=c(1,9), ylim=c(1,11), xlab= "", ylab="")
title(ylab="Reader 3", line=2, cex.lab=1.4, family="Times")
legend(x=0.2, y=12, bty="n", border=NULL, paste("APE",round(Reader11.stats[1,8],1), "\n", "CV", round(Reader11.stats[1,6], 1)))  

par(mai=c(0.35,0.55,0.05,0))
plotAB(ab.tA12,what="bias",col.agree="gray50", xlim=c(1,9), ylim=c(1,11), xlab= "", ylab=" ")
title(ylab="Reader 4", line=2, cex.lab=1.4, family="Times")
legend(x=0.2, y=12,bty="n",paste("APE", round(Reader12.stats[1,8], 1), "\n", "CV", round(Reader12.stats[1,6], 1))) 

mtext("Band Count Average Excluding Reader", cex = 1.5, side=1, line = 0.2, outer=T, at=0.55)

mtext("Spot Shrimp", cex = 2, side = 3, line = 0.2, outer = TRUE, at=0.53)
